        ...
        .directive('pwCanvas', function() {
            return {
                ...
                link: function postLink(scope, elm) {
                    ...
                    if (options.imageSrc) {
                        ...
                        image.onload = function() {
                            if (image.height <= image.width) {
                                ...
                                options.height = window.innerHeight * 0.7;
                    ...
                    scope.$watch('options.imageSrc', function(newValue, oldValue) {
                        if (newValue) {

                            var image2 = new Image();
                            image2.src = newValue;
                            image2.onload = function() {
                                // console.log(image2.height, image2.width);
                                if (image2.height <= image2.width) {
                                    options.width = window.innerWidth;
                                    options.height = image2.height * options.width / image2.width;
                                } else {
                                    options.height = window.innerHeight * 0.7;
                                    options.width = image2.width * options.height / image2.height;
                                }

                                image2.height = options.height;
                                image2.width = options.width;
                                canvas.height = canvasTmp.height = options.height;
                                canvas.width = canvasTmp.width = options.width;

                                // ctx.drawImage(this, 0, 0, image2.width, image2.height);
                    ...
                    var setPointFromEvent = function(point, e) {
                        if (isTouch) {
                            // console.log(e, e.changedTouches[0],getOffset(e.target).left );
